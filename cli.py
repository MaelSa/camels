...
while not end:
    ...
    board = connexion_avec_serveur.recv(255).decode()
    ressources = connexion_avec_serveur.recv(255).decode()
    print(board)
    print(ressources)
    if tour == 'votre':
        hand = connexion_avec_serveur.recv(255).decode()
        ...
        if choice == 'vendre':
            ...
            while not ok_sell:
                ...
                ok_sell = (connexion_avec_serveur.recv(255).decode() == 'true')
            print(connexion_avec_serveur.recv(255).decode())
            ...
            while not ok_pre_trade:
                ...
                while not ok_chameaux:
                    ...
                    ok_chameaux = (connexion_avec_serveur.recv(255).decode() == 'true')
                ok_cards = False
                while not ok_cards:
                    nb_cards = input("Combien de cartes voulez-vous échanger ?\n")
                    ok_cards = (connexion_avec_serveur.recv(255).decode() == 'true')
                ok_pre_trade = (connexion_avec_serveur.recv(255).decode() == 'true')
            ok_trade = False
            while not ok_trade:
                possible_choice = connexion_avec_serveur.recv(255).decode()
                chosen_cards = connexion_avec_serveur.recv(255).decode()
                ...
                while not ok_choice_c:
                    ...
                    connexion_avec_serveur.send(choice.encode())
                    if connexion_avec_serveur.recv(255).decode() == 'true':
                        ok_choice_c = True
                    else:
                        ok_choice_c = False
                ok_trade = (connexion_avec_serveur.recv(255).decode() == 'true')
                print(connexion_avec_serveur.recv(255).decode())
        elif choice == 'prendre':
            ok_take = ""
            while ok_take != 'true':
